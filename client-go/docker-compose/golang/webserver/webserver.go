...
import (
	"context"
	"ingresslist/config"
	"ingresslist/webserver/api"
	"ingresslist/webserver/indexpage"
	"log/slog"
	"net"
	"net/http"
	"time"
...
func Start(ctx context.Context) error {
	...
	slog.Info("Start Server", "hostname", cfg.hostname, "port", cfg.port)
	go func() {
...
func Stop(ctx context.Context) error {
	...
	case <-time.After(time.Microsecond * 500):
		slog.Error("shutdown time out over 500 ms")
		break
	}
	return nil
}
